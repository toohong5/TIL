...
2. user 레코드 생성
   ...
   ```python
   # orm
   user = User(first_name='홍은', last_name='박', age=24, country='경상남도', phone='010-1111-2222', balance=2500000)
   user.save()
   ```

   ```sql
   -- sql
   INSERT INTO users_user VALUES('홍은', '박', 24, '경상남도', '010-1111-2222', 250000);
   ```
...
4. 해당 user 레코드 수정
   ...
   ```
      ...
      ```sql
   -- sql
   UPDATE users_user SET first_name='철수' WHERE rowid=101;
      ```
...
3. 강원도에 사는 사람의 평균 계좌 잔고
   ...
   ```sql
   -- sql
   SELECT AVG(balance) FROM users_user WHERE location='강원도';
   ```
